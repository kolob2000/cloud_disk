...
export const createFolder = createAsyncThunk<void,
    ...
    async (arg, thunkAPI) => {
        ...
        headers.headers['Authorization'] = cookie.get('token')
        const response = await fetch(`http://192.168.0.195:3002/api/cloud/`, headers as RequestInit)
        if (!response.ok) {
...
export const fileUploader = createAsyncThunk<void, FormData, { rejectValue: string, dispatch: AppDispatch }>(
    ...
    async (files, thunkAPI) => {
        ...
        const response = await fetch(`http://192.168.0.195:3002/api/cloud/upload`, {
            method: 'POST',
            body: files,
            headers: {
                'Authorization': cookie.get('token'),
